...
export const CreateProblem = ({ socket, roomId }: { socket: any; roomId: string }) => {
  ...
  const createDemoQuiz = () => {
    toast({
      title: "Demo Quiz is Initialized",
      description: `(You can now start the quiz by going to Quiz Controls - Start Quiz)`,
    })
    socket.emit("createDemoQuiz", { roomId })
  }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <Button onClick={createDemoQuiz} variant="secondary">Create Demo Quiz</Button>
        ...
        <div className="space-y-2">
          <Label htmlFor="image">
            Image <span className="text-red-500 text-xs">Coming Soon</span>
          </Label>
          <Input
            key={fileKey}
            id="image"
